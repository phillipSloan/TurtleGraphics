...
void test(void)
{
   ...
   /*this is now a valid operator for the extension*/
   trtl->cw = 0;
   strcpy(trtl->wds[2], "%");
   assert(polish(trtl, s, 0));

   /*fails with an invalid operator*/
   trtl->cw = 0;
   strcpy(trtl->wds[2], "&");
   assert(!polish(trtl, s, 0));
   assert(resetStack(s));
   ...
   strcpy(trtl->wds[0], ">");
   assert(isSign(trtl) == 1);

   strcpy(trtl->wds[0], ">=");
   assert(isSign(trtl) == 2);

   strcpy(trtl->wds[0], "<");
   assert(isSign(trtl) == 3);

   strcpy(trtl->wds[0], "<=");
   assert(isSign(trtl) == 4);

   strcpy(trtl->wds[0], "==");
   assert(isSign(trtl) == 5);
   ...
   trtl->pen = false;
   strcpy(string, "PEN DOWN");
   testReadString(trtl, string, 2);
   assert(instruction(trtl, s));
   assert(trtl->pen);
   trtl->cw = 0;
   strcpy(trtl->wds[1], "UP");
   assert(instruction(trtl, s));
   assert(!trtl->pen);
   ...
   trtl->cw = 0;
   trtl->tw = 8;
   strcpy(string, "/* this is a comment % $ */");
   testReadString(trtl, string, 8);
   assert(instruction(trtl, s));

   trtl->cw = 0;
   trtl->tw = 8;
   strcpy(string, "/* this is a comment with no end");
   testReadString(trtl, string, 8);
   assert(!instruction(trtl, s));

   trtl->cw = 0;
   trtl->tw = 8;
   strcpy(string, "this is a comment with no start */");
   testReadString(trtl, string, 8);
   assert(!instruction(trtl, s));

   trtl->cw = 0;
   trtl->tw = 13;
   strcpy(string, "/* this at the start of a file */ { FD 10 }");
   testReadString(trtl, string, 13);
   assert(prog(trtl, s));
   ...
   assert(((int)trtl->x) == 400);
   assert(((int)trtl->y) == 300);
   strcpy(trtl->wds[0], "JUMP");
   strcpy(trtl->wds[1], "200");
   strcpy(trtl->wds[2], "200");
   assert(instruction(trtl, s));
   assert(((int)trtl->x) == 200);
   assert(((int)trtl->y) == 200);

   /*ensuring variable A is set to used*/
   trtl->cw = 0;
   trtl->varUsed[0] = true;
   trtl->tw = 6;
   trtl->varArr[0] = 10;
   strcpy(string, "IF A > 5 { }");
   testReadString(trtl, string, 6);
   assert(instruction(trtl, s));

   trtl->varArr[0] = 10;
   trtl->cw = 0;
   strcpy(string, "IF A < 35 { }");
   testReadString(trtl, string, 6);
   assert(instruction(trtl, s));

   trtl->cw = 0;
   strcpy(string, "IF A >= 5 { FD 45 LT 45 }");
   testReadString(trtl, string, 10);
   assert(instruction(trtl, s));

   trtl->cw = 0;
   strcpy(string, "IF A <= 10 { FD 45 LT 45 }");
   testReadString(trtl, string, 10);
   assert(instruction(trtl, s));
