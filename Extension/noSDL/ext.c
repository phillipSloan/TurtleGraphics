/*NOT A PRODUCTION VERSION - SDL HAS BEEN REMOVED TO ALLOW/SHOW TESTING*/
#include "turtle.h"
...
bool drawCircle(turtle* trtl)
{
 ...
 /*x`  Neill_SDL_RenderDrawCircle(trtl->sw.renderer, trtl->x,\
                                 trtl->y, radius);
...
bool set(turtle* trtl, stack* s)
{
   ...
   trtl->cw++;
   if(var(trtl)){
      /*if it's a variable, initialise it as used*/
      varIndex = setVariable(trtl);
      trtl->cw++;
      if (strsame(trtl->wds[trtl->cw], ":=")){
         trtl->cw++;
