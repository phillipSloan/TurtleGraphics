...
bool incLoop(turtle* trtl, stack* s)
{
   ...
   /*pop variables from previous function off the stack*/
   stack_pop(s, &sTo);
   stack_pop(s, &sFrom);
...
bool set(turtle* trtl, stack* s)
{
   ...
   trtl->cw++;
   if(var(trtl)){
      /*if it's a variable, initalise it so it can be used*/
      varIndex = setVariable(trtl);
      trtl->cw++;
      if (strsame(trtl->wds[trtl->cw], ":=")){
         trtl->cw++;
...
int setVariable(turtle* trtl)
{
   char variable;
   int varIndex;
   /*find the variables index within the bool alphabet array*/
...
bool polish(turtle* trtl, stack* s, int varIndex)
{
   ...
   /* if it's an operator, perform arthimetic */
   if(op(trtl)){
   ...
   /* if it is a variable or number, push it to the stack*/
   if(var(trtl)){
...
bool storeVariable(turtle* trtl, stack* s, int varIndex)
{
   ...
   /*if something is still on the stack, valid RPN was not used*/
   if (s->size != empty){
      return false;
   }
   return true;
